...
def py_mysql(sql='SELECT phonenumber from smartraining.sys_user;'):
    conn = pymysql.connect(host="129.211.54.231", user="sdxx_star", password="tPkVeKrrO1*6", port=3306)
    cursor = conn.cursor()
    cursor.execute(sql)
...
def phone_num(num=1):
    ...
    for n in all_phone_nums:
        if n not in use_phone:
            phone.append(n)
    return phone[0]
